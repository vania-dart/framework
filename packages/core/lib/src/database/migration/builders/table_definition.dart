...
class TableDefinition implements Future<void> {
  ...
  /// Memoised so the migration runs once no matter how many `Future` members
  /// are used on the same instance (`await`, `catchError`, `whenComplete` …).
  Future<void> _getExecutionFuture() {
    return _execution ??= () async {
  ...
  Future<void> _applyTableOptions() async {
    ...
    // The adapter decides which options its engine can express, so a
    // migration carrying MySQL-only options still runs on the other drivers.
    final statements = adapter.renderTableOptions(
      _tableName,
      engine: _engine,
      charset: _charset,
