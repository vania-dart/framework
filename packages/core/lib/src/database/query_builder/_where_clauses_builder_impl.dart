...
import '../impl/legacy_query_builder.dart' show QueryBuilder, QueryCallback;
import '../utils/singularize.dart';
...
abstract mixin class WhereClausesBuilderImpl implements QueryBuilder {
  /// Allocates placeholder names for this builder's bindings.
  ///
  /// Per-instance, and only ever moves forward: two builders running
  /// concurrently must not hand out the same name, and a builder that is
  /// still being chained must not reuse a name it has already bound.
  int _paramCounter = 0;
  ...
  /// Delegates to the shared guard so WHERE and HAVING accept exactly
  ...
  QueryBuilder orWhere(
    ...
    if (condition is String) {
      ...
      _paramCounter = nested.currentParamCounter;
    } else {
